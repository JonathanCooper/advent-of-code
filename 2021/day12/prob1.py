...
def find_paths(node, seen=None, path_so_far=None):
    ...
    if node.name == 'end':
        ...
        return 1
    if node.is_small() and node.name in seen:
        #print(f'already saw small node: {node.name}')
        return 0
    else:
        seen.add(node.name)
        path_so_far.append(node.name)
    for child in node.children:
        paths += find_paths(child, seen.copy(), path_so_far.copy())
    return paths
